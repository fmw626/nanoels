// https://github.com/fmw626/nanoels forked from
...
const int ENCODER_STEPS_INT = 1200; // 600 step x2 spindle optical rotary encoder. Fractional values not supported.
...
const long MAX_TRAVEL_MM_Z = 400; // Lathe bed doesn't allow to travel more than this in one go, 30cm / ~1 foot
const long BACKLASH_DU_Z = 4250; // 0.425mm backlash in deci-microns (10^-7 of a meter)
const char NAME_Z = 'Z'; // Text shown on screen before axis position value, GCode axis name

// Cross-slide lead screw (X) parameters.
const long SCREW_X_DU = 10000; // 1.00mm lead screw with 3x reduction in deci-microns (10^-7) of a meter
...
const long MAX_TRAVEL_MM_X = 120; // Cross slide doesn't allow to travel more than this in one go, 10cm
const long BACKLASH_DU_X = 1750; // 0.175mm backlash in deci-microns (10^-7 of a meter)
...
const long SPEED_START_A1 = 400; // Initial speed of a motor, steps / second.
const long ACCELERATION_A1 = 16000; // Acceleration of a motor, steps / second ^ 2.
const long SPEED_MANUAL_MOVE_A1 = 1600; // Maximum speed of a motor during manual move, steps / second.
const bool INVERT_A1 = true; // change (true/false) if the carriage moves e.g. "left" when you press "right".
...
const long BACKLASH_DU_A1 = 150; // 0,015mm backlash comp on wormgear. calibrated 17.08.23
